import { addCategory, deleteCategory } from "@/Redux/slices/constructDaySlice";
      ...
      <div className=" flex flex-wrap">
        {category.map((categ: categoryType) => (
          <div
            className={` ${
              "border-" + color + "-400"
            } flex justify-between items-center min-w-44 xl:min-w-52 p-2 rounded-xl border-2 mr-2 mb-2 `}
            key={categ.id}
          >
            <p>{categ.categ}</p>
            <FaDeleteLeft
              onClick={() => dispatch(deleteCategory(categ.id))}
...
export default CategoryConstruct;
